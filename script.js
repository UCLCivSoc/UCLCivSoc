...
function loadReport(i) {
    ...
    document.getElementById('report-content').innerHTML = `<div class="report-header"><h2>Weekly Report</h2><span class="date">${r.label}</span></div><div class="stat-grid"><div class="stat-box"><span class="stat-number">${r.stats.attendance}</span><span class="stat-label">Present</span></div><div class="stat-box"><span class="stat-number">${r.stats.spent}</span><span class="stat-label">Spent</span></div><div class="stat-box"><span class="stat-number">${r.stats.discussion}</span><span class="stat-label">Topics</span></div><div class="stat-box"><span class="stat-number">${r.stats.decisions}</span><span class="stat-label">Decisions</span></div></div>${fin ? `<div class="data-section"><h3>Treasury</h3><table class="transparency-table">${fin}</table></div>` : ''}<div class="data-section"><h3>Minutes</h3><ul>${r.minutes.map(m=>`<li>${m}</li>`).join('')}</ul></div>`;
}
